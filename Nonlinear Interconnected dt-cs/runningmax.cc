...
int main() {
  ...
  auto dyn = [K](state_type& x, const control_type u, const exog_type w) -> state_type {
    state_type xn;
    xn[0] = std::min(K*(x[0] + u[0]), w[0] + 1);
    xn[0] = saturate(x[0], 0, 32.0);
    return xn;
  };
  ...
  */
  auto sys_overapprox = [dyn](const input_type i_ll, const input_type i_ur, state_type& o_ll, state_type& o_ur){
    double eta_x[sDIM] = {h_eta*0.5};
    state_type r;

    std::vector<state_type> successors;

    // Determine grid points per dimension
    int grid_pts_per_dim = static_cast<int>(std::round(std::pow(samples, 1.0 / sDIM)));
    ...
    for (int i = 0; i < sDIM; ++i) {
        ...
        for (int j = 0; j < grid_pts_per_dim; ++j) {
            double center = o_ll[i] - eta_x[i] + (j + 0.5) * step;
    ...
    recurse = [&](int dim, state_type& current_sample) {
        if (dim == sDIM) {
            successors.push_back(dyn(current_sample, {i_ll[1]}, {i_ur[2]}));
            return;
        }
    ...
    if (successors.size() > samples) {
        successors.resize(samples);
    }
